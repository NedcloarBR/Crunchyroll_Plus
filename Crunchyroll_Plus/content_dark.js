...
class StyleSwitcher {
  constructor() {
    ...
    this.link.href = chrome.extension.getURL('styles_dark.css');
    ;
...
const styleSwitcher = new StyleSwitcher(); // assume we're enabled until we hear back
...
port.onMessage.addListener(status => {
  ...
  if (typeof status === 'boolean') {
    ...
    console.error('Dark-Crunchyroll: bad response', status); // keep asking until we get a good response
